...
SecondaryStructure MOIP::solve_objective(int o, double min, double max)
{
    ...
    env_.out() << endl
               << "Solution status = " << cplex_.getStatus() << endl;
    env_.out() << endl
               << "Objective value = " << cplex_.getObjValue() << endl;
    ...
    for (size_t i = 0; i < insertion_sites_.size(); i++)
    {
        ...
        if (insertion_values[index_of_first_components[i]])
    ...
    for (size_t u = 0; u < rna_.get_RNA_length() - 6; u++)
    {
        ...
        {
            if (allowed_basepair(u, v))
                if (basepair_values[get_yuv_index(u, v)])
...
void MOIP::add_problem_constraints(const IloModel &model_)
{
    ...
    for (u = 0; u < n; u++)
    {
        ...
        for (size_t i = 0; i < insertion_sites_.size(); i++)
        {
            ...
            {
                Component &c = x.comp[j];
                if (u >= c.pos.first and u <= c.pos.second)
                { // Cxip contains u
                    c4 += C(i, j);
                    nterms++;
                }
...
bool RNA::allowed_basepair(size_t u, size_t v) const
{
    ...
    if (b - a < 4)
        return false;
    if (a >= rna_.get_RNA_length() - 6)
        return false;
    if (b >= rna_.get_RNA_length())
        return false;
    if (get_yuv_index(a, b) == rna_.get_RNA_length() + 1)
