...
public:
  ...
  void resize(int n) {
    N_ = n;
    V_.resize(static_cast<unsigned long>(N_) * (N_ - 1) * (N_ - 2) * (N_ - 3) /
              2 / 3 / 4); // This number can be HUGE
  }
  void fill(const float &v) { std::fill(V_.begin(), V_.end(), v); }
  float &operator()(int i, int d, int e, int j) {
    return V_[index(i, d, e, j)];
  }
  const float &operator()(int i, int d, int e, int j) const {
    return V_[index(i, d, e, j)];
  }
  ...
  int index(int h, int r, int m, int s) const {
    ...
    return (h == r && m == s)
               ...
               : (-24 - 50 * h - 35 * h2 - 10 * h3 - h4 - 36 * m - 12 * m2 +
                  12 * n + 70 * h * n + 30 * h2 * n + 4 * h3 * n + 24 * m * n -
                  12 * n2 - 30 * h * n2 - 6 * h2 * n2 + 4 * h * n3 + 44 * r -
                  48 * n * r + 12 * n2 * r + 24 * r2 - 12 * n * r2 + 4 * r3 +
                  24 * s) /
                     24;
  }
  std::vector<float> V_;
...
public:
  ...
  void fill(const float &v) { std::fill(V_.begin(), V_.end(), v); }
  float &operator()(int i, int d, int e, int s) {
    return V_[index(i, d, e, s)];
  }
  const float &operator()(int i, int d, int e, int s) const {
    return V_[index(i, d, e, s)];
  }
...
private:
  int index(int i, int h1, int m1, int s) const {
    ...
    assert(d - 6 >= s);
    assert(i < h1);
    return i * d5 * d1d2 / 2 + s * d1d2 / 2 + h1_i_1 * (d - 1) -
           h1_i_1 * (h1 - i) / 2 + m1 - h1 - 1;
  }
...
public:
  ...
  void get_posterior(vector<float> &bp1, vector<float> &bp2,
                     vector<int> &offset) const;

private:
  void fastiloops(int i, int j, DPtable4 &Qg, DPtableX &Qx, DPtableX &Qx2);
  void fastiloops_pr(int i, int j, DPtable4 &Qg, DPtableX &Qx, DPtableX &Qx2,
                     DPtable4 &Pg, DPtableX &Px, DPtableX &Px2);
