...
typedef struct Comp_
{
  ...
  int score;
  size_t k;
  Comp_(pair<int, int> p, int s) : pos(p), score(s)
  {
...
typedef Eigen::TensorFixedSize<float, Eigen::Sizes<4>> tensor44;
...
private:
  ...
  compute_ON5_PK_partition_function(void);
  void compute_ON6_PK_posterior(void);
  void get_posterior(vector<float> bp_proba, vector<int> offset);
  void fastILloops(uint i, uint j, tensor44 &Qg, tensor44 &Qx, tensor44 &Qx2);
...
inline pair_t RNA::pair_type(int i, int j) const
{
  return pair_map[bseq_[i]][bseq_[j]];
}
